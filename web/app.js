...
form.onsubmit=async event=>{event.preventDefault();try{lastInput=input();render(await request('/api/plan',lastInput));loadSaved()}catch(error){showError(error.message)}};
document.querySelector('#refine').onclick=async()=>{const revision=document.querySelector('#revision').value.trim();if(!revision)return;try{render(await request('/api/refine',{input:lastInput,request:revision}));loadSaved()}catch(error){showError(error.message)}};
...
async function loadSaved() {
  try {
    ...
    listEl.innerHTML=projects.map(item=>`<button data-id="${item.id}"><b>${escapeHtml(item.project_name)}</b><small>${escapeHtml(item.created_at)}</small></button>`).join('');
    listEl.querySelectorAll('button').forEach(button=>button.onclick=async()=>{const saved=await request(`/api/projects/${button.dataset.id}`);lastInput=saved.input;render(saved.result)});
  } catch (_) {}
}
