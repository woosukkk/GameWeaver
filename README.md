...
첫 실행 후 이메일과 8자 이상의 비밀번호로 계정을 만듭니다. 비밀번호는 PBKDF2-SHA256 해시로, 로그인 세션은 원문이 아닌 해시로 MySQL에 저장됩니다. 계획·회고·공수 데이터는 로그인한 소유자 기준으로만 조회하거나 변경할 수 있습니다. HTTPS 운영 환경에서는 `.env`에 `APP_ENV=production`을 설정해 세션 쿠키의 `Secure` 속성을 활성화합니다.

인증 API는 이메일 기준 5회, IP 기준 20회의 5분 로그인 시도 제한을 적용합니다. 상태 변경 요청은 CSRF 토큰을 검증하고 만료된 세션은 앱 시작과 로그인 시 정리합니다. 현재 시도 제한은 단일 서버 프로세스용이므로 여러 서버 인스턴스로 확장할 때 Redis 같은 공유 저장소로 옮겨야 합니다.

서버는 각 요청에 `X-Request-ID` 응답 헤더를 반환하고 표준 오류 출력에 JSON 한 줄 로그를 기록합니다. 로그에는 요청 ID, HTTP 메서드, 경로, 상태 코드, 처리 시간과 인증된 사용자 ID만 포함하며 요청 본문·쿠키·비밀번호·API 키는 기록하지 않습니다.
...
현재 버전은 Single Planning Agent와 규칙 기반 fallback을 모두 실행할 수 있는 MVP다.
