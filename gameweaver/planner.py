...
from .catalog import COMMON_RULES, DEFAULT_GENRE, GENRES
...
def _normalize(payload):
    project = _require(payload, "project", dict)
    members = _require(payload, "members", list)
    ...
    for index, member in enumerate(normalized["members"], 1):
...
def build_harness(project):
    genre = GENRES.get(project["genre"], DEFAULT_GENRE)
    weeks = project["duration_weeks"]
    contexts = [f"{project['engine']} Development", f"{project['dimension']} Game", f"{project['genre']} Context"]
    contexts += ["Small Team", f"{weeks}-Week Development", f"{project['goal']} Context"] + genre["context"]
    ...
    tools = ["calculate_skill_match", "calculate_workload", "check_schedule", "check_task_dependency", "validate_assignment"]
    if weeks > 6:
        tools.append("check_role_balance")
    return {"contexts": list(dict.fromkeys(contexts)), "rules": rules, "tools": tools, "workflow": genre["workflow"], "role_candidates": genre["roles"], "validation_conditions": ["mandatory_tasks_assigned", "members_exist", "workload_within_capacity", "dependencies_valid", "schema_valid"], "permissions": ["read_project_input", "use_registered_tools", "propose_assignments"]}


def _tasks(project):
    catalog = GENRES.get(project["genre"], DEFAULT_GENRE)["tasks"]
    names, result = {}, []
    for index, (name, category, skills, hours, dependencies) in enumerate(catalog, 1):
        task_id = f"task_{index:02d}"
        names[name] = task_id
        resolved = {project["engine"] if skill == "ENGINE" else skill: level for skill, level in skills.items()}
        result.append({"id": task_id, "name": name, "category": category, "required_skills": resolved, "estimated_hours": hours, "dependencies": dependencies, "mandatory": True})
    for task in result:
        task["dependencies"] = [names[name] for name in task["dependencies"]]
